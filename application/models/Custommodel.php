...
class Custommodel extends CI_Model 
{
	public function get_agent_list($param = FALSE)
	{
		...
		$this->db->join('agent_majors', 'agent_majors.agent_id = agent.id', 'LEFT');
		...
		if( !empty( $param['majors'] ) )
		{
			$this->db->where_in('agent_majors.majors_id', $param['majors']);
		}
	...
	public function get_majors($agent_id = FALSE)
	{
		$this->db->select('majors.*');
		$this->db->from('majors');
		$this->db->join('agent_majors', 'agent_majors.majors_id = majors.id', 'LEFT');
		if( $agent_id )
		{
			$this->db->where('agent_majors.agent_id', $agent_id);
		}
		return $this->db->get()->result();
	}
